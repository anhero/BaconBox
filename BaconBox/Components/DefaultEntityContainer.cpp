...
namespace BaconBox {
    ...
    void DefaultEntityContainer::propagateMessage(int senderID, int destID, int message, void *data){
        SendMessageChild messageStruct(senderID, destID, message, data);
		EntityContainerLooper::forEachChildCurrentFrame(this, messageStruct);
    }
	...
	void DefaultEntityContainer::deleteAllChildren(){
	    std::set<MovieClipEntity*> toDelete;
	    for(ChildArray::iterator i = children.begin(); i != children.end(); i++){
			for(std::deque<MovieClipEntity*>::iterator j = i->second.begin(); j != i->second.end(); j++){
				toDelete.insert(*j);
	...
	void DefaultEntityContainer::setFrame(int frame){
		...
		if(currentChildren){
			for(std::deque<MovieClipEntity*>::iterator i = currentChildren->begin(); i != currentChildren->end(); i++){
				reinterpret_cast<DefaultMatrix*>((*i)->getMatrixComponent())->setFrameMatrix(frame);
                reinterpret_cast<DefaultColorTransform*>((*i)->getColorTransform())->setFrameColorTransform(frame);
			}
		}
		else {
			children[frame];
