...
namespace BaconBox {
	...
	Mesh::Mesh() : Component(), mustSync(false), transform(NULL){
...
//	}
	...
	void Mesh::syncMesh(){
	    ...
	    if(transform)postTransformVertices.multiplyWithMatrix(transform->getMatrix());
	}
	...
	void Mesh::receiveMessage(int senderID, int destID, int message, void *data) {
		if (senderID == Transform::ID && destID == Entity::BROADCAST) {
			...
			else if (message == Transform::MESSAGE_MATRIX_CHANGED) {
			    mustSync = true;
			}
		}
		else if(destID == Mesh::ID && message == Entity::MESSAGE_ADD_COMPONENT){
			this->initializeConnections();
			mustSync = true;
		}
