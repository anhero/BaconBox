...
function LuaState:init(params, own, stateConstructor)
	...
	if type(params) == "table" then
		entity = stateConstructor(unpack(params))
	else
		entity = stateConstructor(params)
	end
...
function printUserData(userdata)
	...
	print("Printing functions of", userdata)
	local to_print = {}
...
%{
	...
	///////////////////////
	#if defined(BB_LUA)
		swig_type_info * getTypeByName(swig_module_info *start, swig_module_info *end, const char *name) {
